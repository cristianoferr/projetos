...
Public Class util
    ...
    Public Function getParDireto(ByVal param As String, ByVal arquivo As String) As String
        Try
            ...
            For Each sLine In params
                'Console.WriteLine(sLine)
                If hashInterno <> "" Then
                    sLine = LCase(getEncrypt(Data_Hex_Asc(LCase(sLine)), hashInterno))
                    'param = LCase(Data_Asc_Hex(getEncrypt(param, hashInterno)))
                End If
                If InStr(sLine, param + "=") Then
                    Dim d As String = Mid(sLine, InStr(LCase(sLine), "=") + 1, Len(sLine))
    ...
    Sub saveWebPage(ByVal url As String, ByVal file As String)
        Try
        ...
        Catch ex As Exception
            Console.WriteLine("Erro baixando" & url)
        End Try
    ...
    Public Function ReadFileAttributes(ByVal file As String) As String
    ...
    Public Sub executarComando(ByVal comando As String)
        ...
        'Application.DoEvents()
        ChDir(workDir)
        Shell(workDir + "\exec.bat", AppWinStyle.NormalFocus, True)
