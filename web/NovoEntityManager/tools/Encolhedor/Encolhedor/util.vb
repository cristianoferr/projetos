...
Public Class util
    ...
    Public Sub setClearcaseDir(cc As String)
        clearcaseDir = cc
    End Sub

    Public Function getClearcaseDir() As String
        Return clearcaseDir
    End Function
    ...
    Public Function geraPass(ByVal hash As String)
        Return geraPass(hash, DateTime.UtcNow)
    End Function

    Public Function geraPassAntiga(ByVal hash As String, ByVal dias As Integer)
        Dim data As DateTime = DateTime.UtcNow
        data = data.AddDays(dias)
        Return geraPass(hash, data)
    End Function

    Public Function geraPass(ByVal hash As String, ByVal hoje As DateTime)
        Dim ret As String = ""
        Dim i
        Dim tot = 0
    ...
    Function listaLabel(rotulo As String, vobDir As String) As List(Of String)
        Dim objStreamWriter As StreamWriter
        Dim rand As Integer = Rnd() * 1000
        'Pass the file path and the file name to the StreamWriter constructor.
        objStreamWriter = New StreamWriter(workDir + "\exec" & rand & ".bat")

        'Write a line of text.

        Dim file As String = workDir + "\saida" & rand & ".tmp"

        Dim comando As String = """" & clearcaseDir + "cleartool.exe"" find " & vobDir & " -version lbtype(" & rotulo & ") -print > " + file
        objStreamWriter.WriteLine("del " & workDir & "\*.tmp")
        objStreamWriter.WriteLine(comando)
        'objStreamWriter.WriteLine("del " & txtWorkDir.Text + "exec.bat")
        objStreamWriter.Close()

        Shell(workDir + "\exec" & rand & ".bat", AppWinStyle.MinimizedFocus, True)
        Dim d = 0
