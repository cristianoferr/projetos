...
function atualizaGenericoComValor(valor, nome_coluna, idRegistro, flagTexto, modulo) {
    ...
    $.ajax({
        url: '/ConstruBook/ajax/updateEntity.php',
        data: data,
        type: 'post',
...
function atualizaColuna(element, id_sql_coluna, id_coluna, flagTexto) {
    ...
    xmlhttpPost("/ConstruBook/ajax/updateEntity.php", "campo=" + id_sql_coluna + "&flagtexto=" + flagTexto + "&id_coluna=" + id_coluna + "&novoValor=" + element.value, function() {
        if (self.xmlHttpReq.readyState == 4) {
