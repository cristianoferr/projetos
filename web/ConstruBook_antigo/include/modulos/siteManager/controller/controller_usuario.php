    ...
    function efetuaLogin($login, $password, $openId = null) {
       ...
       // write("2");
        ...
        if ($openId == null) {
          ...
          // write("login:$login senha:$password");
            $arrLogin = executaQuerySingleRow("select id_usuario,nm_usuario,email_usuario,conta_acesso_usuario from usuario where email_usuario=? and senha_usuario=?", array($login, $password));
        } else {
            $password = $openId->identity;
            $arrLogin = executaQuerySingleRow("select id_usuario,nm_usuario,email_usuario,conta_acesso_usuario from usuario where email_usuario=?", array($login));
        }
