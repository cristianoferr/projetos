...
define("DOMAIN", "construbook.com");
define("AMBIENTE_PRODUCAO", "server");
define("AMBIENTE_HOMOLOG", "homolog");
define("AMBIENTE_LOCAL", "windows");

if ((strpos(getPaginaAtual(), "127.0.0.1")) || (strpos(getPaginaAtual(), "localhost"))) {
    $url = "127.0.0.1";
    define("AMBIENTE", AMBIENTE_LOCAL);
} else if ((strpos(getPaginaAtual(), "192.168.1.109"))) {
    $url = "192.168.1.109";
    define("AMBIENTE", AMBIENTE_LOCAL);
} else if ((strpos(getPaginaAtual(), "site90.com"))) {
    $url = "construbook.site90.com";
    define("AMBIENTE", AMBIENTE_HOMOLOG);
} else {
...
} else if (AMBIENTE == AMBIENTE_HOMOLOG) {
    define("SITE_URL", "http://$url" );
} else {
