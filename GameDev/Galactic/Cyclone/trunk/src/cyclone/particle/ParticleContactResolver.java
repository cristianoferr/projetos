...
public class ParticleContactResolver {
    ...
    public ParticleContactResolver(int iterations){
        ...
        */
      public  void resolveContacts(Vector<ParticleContact> contactArray,
            double duration){
    	   int i;
    	    ...
    	    while(iterationsUsed < iterations)
    	    {
    	        ...
    	        for (i = 0; i < contactArray.size(); i++)
    	        {
    	            if (contactArray.elementAt(i).particle[0] == contactArray.elementAt(maxIndex).particle[0])
    	            {
    	                contactArray.elementAt(i).penetration -= move[0].scalarProduct(contactArray.elementAt(i).contactNormal);
    	            }
    	            else if (contactArray.elementAt(i).particle[0] == contactArray.elementAt(maxIndex).particle[1])
    	            {
    	                contactArray.elementAt(i).penetration -= move[1].scalarProduct(contactArray.elementAt(i).contactNormal);
    	            }
    	            if (contactArray.elementAt(i).particle[1]!=null)
    	            {
    	                if (contactArray.elementAt(i).particle[1] == contactArray.elementAt(maxIndex).particle[0])
    	                {
    	                    contactArray.elementAt(i).penetration += move[0].scalarProduct(contactArray.elementAt(i).contactNormal);
    	                }
    	                else if (contactArray.elementAt(i).particle[1] == contactArray.elementAt(maxIndex).particle[1])
    	                {
    	                    contactArray.elementAt(i).penetration += move[1].scalarProduct(contactArray.elementAt(i).contactNormal);
    	                }
    	            }
