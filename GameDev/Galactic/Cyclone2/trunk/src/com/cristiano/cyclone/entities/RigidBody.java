package com.cristiano.cyclone.entities;
...
public class RigidBody {
	...
	/**
	 * The resultant transform of the primitive. This is calculated by combining
	...
	/**
	 * Holds the inverse of the body's inertia tensor. The intertia tensor
	...
	private boolean flagGravityEmitter=false;;
	...
	// Essa funcao serve para determinar se o objeto produz gravidade (tiros nao
	// precisam checar, por ex)
	...
	/**
	 * This is a convenience function to allow access to the axis vectors in the
	 * transform for this primitive.
	 */
	...
	public void calculateDerivedData() {
		...
		HelperFunctions._calculateTransformMatrix(transformMatrix, position,
				orientation);
	...
	public void integrate(double duration) {
		...
		if (velocity.magnitude() >= PhysicsConsts.C*0.9) {
			velocity.normalise(PhysicsConsts.C*0.9);
		}
		...
		// Impose drag.
		// velocity.multiVectorScalar(Math.pow(CycloneConfig.getAngularDamping(),
		// duration));
		rotation.multiVectorScalar(Math.pow(CycloneConfig.getAngularDamping(),
				duration));
		...
		if (canSleep) {
			...
			if (motion < CycloneConfig.getSleepepsilon()) {
				setAwake(false);
	...
	public void setMass(double mass) {
		...
		case POLYGON:
			tensor = new Matrix3();
			tensor.setBlockInertiaTensor(((GeomBox) getGeom()).halfSize,
					getMass());
		...
		case SPHERE:
			tensor = new Matrix3();
			double coeff = 0.4f * getMass()
					* ((GeomSphere) getGeom()).getRadius()
					* ((GeomSphere) getGeom()).getRadius();
	...
	public double getRelativisticMass() {
		return getMass()
				/ (1 - (Math.sqrt(Math.pow(velocity.magnitude(), 2)
						/ Math.pow(PhysicsConsts.C, 2))));
	}
	 ...
	 * CycloneConfig.linearDamping = linearDamping; CycloneConfig.angularDamping
	 * = angularDamping; }
	 */
	...
	void fillsTransform(Matrix4 transform) {
		System.arraycopy(transformMatrix.data, 0, transform.data, 0,
				transformMatrix.data.length);
		/*
	...
	void fillsTransform(double matrix[]) {
		System.arraycopy(transformMatrix.data, 0, matrix, 0,
				transformMatrix.data.length);
	 ...
	 */
	/*
	 * void getVelocity(Vector3 velocity) { velocity = this.velocity; }
	 */
	 ...
	 */
	/*
	 * void getLastFrameAcceleration(Vector3 linearAcceleration) { acceleration
	...
	public void setGravityEmitter(boolean b) {
		flagGravityEmitter=b;
		
	}
	public boolean isGravityEmitter(){
		return flagGravityEmitter;
	}
