...
namespace Rimworld.Utilities.Pathfinding
{
    public class Path_TileGraph
    {
        ...
        public Path_TileGraph(World world)
        {
            ...
            Utils.Log("Path_TileGraph: Created " + nodes.Count + " nodes.");
        ...
        bool IsClippingCorner(Tile curr, Tile neigh)
        {
            ...
            if (Mathf.Abs(dX) + Mathf.Abs(dY) == 2)
            {
                ...
                if (World.map.GetTileAt(curr.X - dX, curr.Y).movementCost == 0)
                {
                    // East or West is unwalkable, therefore this would be a clipped movement.
                    return true;
                }

                if (World.map.GetTileAt(curr.X, curr.Y - dY).movementCost == 0)
                {
